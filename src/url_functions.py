...
def get_content_(url): #Gửi HTTP request đến URL, giả lập trình duyệt để tránh bị chặn
    """
...
def get_content_news_from_news_url(url: str) -> dict: #Trích xuất nội dung một bài báo (title, description, content)
    """
    Example Args:
...
def get_news_links_from_sub_topic_page_link(sub_topic_page_link: str) -> list: #Trích xuất tất cả link bài báo từ 1 trang con (vd: /the-gioi-p2)
...
def get_page_urls_from_sub_topic_url(sub_topic_url, pages=1) -> list: #Tạo danh sách URL của các trang phân trang
...
def get_all_news_urls_from_topics_links(topics_links: dict, n_pages_per_topic=1) -> dict: #Hàm tổng hợp: crawl link bài viết từ nhiều chủ đề khác nhau
